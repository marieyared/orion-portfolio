...
from supabase_client import sign_in, sign_up, load_portfolio, save_portfolio, save_snapshot
...
# ══════════════════════════════════════════════════════════════════
# SCREEN 0 — AUTH (login / signup)
# ══════════════════════════════════════════════════════════════════
def screen_auth():
    st.markdown('<div class="orion-logo">ORION / PORTFOLIO INTELLIGENCE</div>', unsafe_allow_html=True)
    st.markdown('<div class="orion-headline">Welcome to Orion</div>', unsafe_allow_html=True)
    st.markdown('<div class="orion-sub">Sign in to access your portfolio, or create a free account.</div>', unsafe_allow_html=True)
        ...
        if st.button("Sign in", use_container_width=True, key="btn_login"):
            if not email or not password:
                ...
                try:
                    ...
                    portfolio = load_portfolio(session.access_token, session.refresh_token, user.id)
                    if portfolio:
                        st.session_state.entry_rows = portfolio["entry_rows"]
                        st.session_state.cash_rows  = portfolio["cash_rows"]
                        st.session_state.bond_rows  = portfolio["bond_rows"]
                    st.session_state.screen = "entry"
                    st.rerun()
            ...
            if st.session_state.user_id:
                try:
                    ...
                    save_snapshot(
                        ...
                        st.session_state.refresh_token,
                        st.session_state.user_id,
                        sum(h["current"] for h in valid),
                        sum(h["paid"]    for h in valid),
                    )
                except Exception:
                    pass
            st.rerun()
...
if st.session_state.user_id is None:
    screen_auth()
elif st.session_state.screen == "entry":
    screen_entry()
